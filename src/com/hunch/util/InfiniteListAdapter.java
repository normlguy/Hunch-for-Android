...
/**
 * This adapter provides all of the facilities needed for infinitely loading
 * lists.
 * 
 * Make sure you call tryLoadInline( int ) from getView(), otherwise the list
 * won't know when to append more items. It is also recommended you override
 * shouldLoadInline( int, int ) to decide when to append more items to the list.
 * 
 * @author Tyler Levine
 * @since Mar 15, 2010
 * 
...
public abstract class InfiniteListAdapter< T > extends BaseAdapter
{
	 ...
	 * 
	 * @param items
	 *            The list of items that backs this Adapter.
	 */
	 ...
	 * @param items
	 *            The list of items that backs this Adapter.
	 * @param itemsShownOnLoad
	 *            The number of items displayed at first.
	 * @param itemsAddedOnExpansion
	 *            The number of items added to the list on each expansion.
	 */
	...
	/**
	 * When you inherit from this class, call this method in your getView()
	 * method. The list will not expand otherwise.
	 * 
	 * This method calls shouldLoadInline( int, int ) to decide if more items
	 * should be loaded.
	 * 
	 * @param curPos
	 *            the current view position in the list
	 */
	 ...
	 * 
	 * @param curPos
	 *            The index of the last item visible in the list.
	 * @param size
	 ...
	 * visible or not) to subclasses. Avoid using this unless you need to access
	 * elements of the list that are not currently displayed in the list. You
	 * can use getItem( int ) for most purposes.
	 * 
	...
	/**
	 * This method must be overridden by any sub class (duh, it's abstract). It
	 * generates the views for each item that will appear in the list.
	 * 
	 * It is important to call <code>super.tryLoadInline( int )</code> in this
	 * method to let the list try to expand when appropriate. You can decide
	 * when to expand by overriding <code>shouldLoadInline( int, int )</code>
	 * 
